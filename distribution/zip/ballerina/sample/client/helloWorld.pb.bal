package client;
import ballerina.net.grpc;

public connector helloWorldBlockingStub(string host, int port){
 endpoint<grpc:GRPCConnector> ep {

        create grpc:GRPCConnector(host, port, "blocking", descriptorKey, descriptorMap,op);
    }


    action hello (string req) (string, error) {
        var res, err1 = ep.blockingExecute("helloWorld/hello", req);
        if (err1 != null) {
            error e = {message:err1.message};
            return null, e;
        }
        var response, err2 = (string)res;
        if (err2 != null) {
            error e = {message:err2.message};
            return null, e;
        }
        return response, null;
    }
}
grpc:SSL sslV = {
trustStoreFile:"/home/yasara/Desktop/PR_ORG/ballerina/distribution/zip/ballerina/resources/security/ballerinaTruststore.p12",
trustStorePassword:"ballerina"
};
grpc:Options op = {ssl:sslV};
const string descriptorKey = ".helloWorld.proto";
map descriptorMap ={
".helloWorld.proto":"",
"google.protobuf.google/protobuf/wrappers.proto":""};
